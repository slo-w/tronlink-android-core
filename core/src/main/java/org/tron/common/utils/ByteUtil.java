...
public class ByteUtil {
    ...
    public static byte[] appendBytes(byte[] bytes, byte[] b, int bIndex, int bSize) {
        byte[] result = Arrays.copyOf(bytes, bytes.length + bSize);
        System.arraycopy(b, bIndex, result, bytes.length, bSize);
        return result;
    }
